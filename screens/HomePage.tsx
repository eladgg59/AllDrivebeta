// Landing: header, hero CTA, features; theme + hover animations (web).
...
export default function App() {
  ...
  const hoverAnim = useRef(new Animated.Value(0)).current; // 0 idle, 1 hover (web)
  ...
  const handleHover = (isHovering) => {
    Animated.spring(hoverAnim, {
      ...
      useNativeDriver: false, // color/opacity interpolations
      friction: 8,
      tension: 40,
    }).start();
  };
  ...
  return (
    <Animated.View style={[styles.container, { backgroundColor }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          >
            <Animated.View
              style={[
                styles.loginAuraGlow,
                {
                  opacity: loginShadowOpacity,
          ...
          >
            <Animated.View
              style={[
                styles.signupAuraGlow,
                { opacity: signupAuraOpacity, transform: [{ scale: signupBtnScale }], backgroundColor: blueColor },
              ]}
            />
      ...
      <View style={styles.content}>
        ...
        <Animated.Text style={[styles.description, { color: textColor }]}>
          Our service combines multiple Google Drive accounts into one easy-to-manage workspace.{'\n'} Access and organize all your files from a single dashboard.
        </Animated.Text>
        <View style={styles.buttonContainer}>
          ...
          >
            <Animated.View
              style={[styles.auraGlow, { opacity: auraOpacity, transform: [{ scale: btnScale }], backgroundColor: blueColor }]}
            />
...
// Pressable + dim on press (theme icon)
...
// Feature row; borderColor = string | Animated node
function FeatureCard({ icon, title, desc, textColor, borderColor, backgroundColor }) {
  ...
  return (
    <Pressable onHoverIn={() => handleHover(true)} onHoverOut={() => handleHover(false)} style={styles.cardWrapper}>
      <Animated.View style={[styles.cardAura, { opacity: auraOpacity, backgroundColor: '#3b82f6' }]} />
      <Animated.View
        style={[
          styles.featureCard,
          {
            backgroundColor: backgroundColor,
            borderColor: borderColor.interpolate ? borderColor : borderColor,
            transform: [{ scale }],
          },
        ]}
      >
